  ...
  export default function Contact() {
    ...
    const showListInvite = async () => {
      try {
        ...
        const response = await getApiNoneToken("/getDetailsByPhone/" + currentUserPhone);
            ...
            {inviteList.map((item, id) => (
              <View key={id} style={styles.inviteContainer}>
                ...
                <View style={styles.actions}>
                  <TouchableOpacity onPress={() => acceptInvite(item.id,item.name,item.phone, item.avatar)} style={[styles.button, styles.acceptButton]}>
